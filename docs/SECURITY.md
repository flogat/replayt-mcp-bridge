...
- **Layer bridge knobs for persistence only** — **`REPLAYT_MCP_BRIDGE_STORE_HINT_ROOTS`**, **`REPLAYT_MCP_BRIDGE_RUN_EVENT_FIELDS`**, **`REPLAYT_MCP_BRIDGE_REDACT_RUN_EVENTS`**, and (once implemented) **`REPLAYT_MCP_BRIDGE_RUN_EVENTS_MAX_COUNT`** / **`REPLAYT_MCP_BRIDGE_RUN_EVENTS_MAX_TOTAL_BYTES`** constrain **`persistence_list_run_events`** (paths, top-level event keys, key-based redaction, response volume). They complement host policy but **do not replace** tool-level access control—see [Environment variables](#environment-variables).
...
- **Successful payloads** — Contract snapshots, Mermaid text, dry-check reports, and especially **persistence events** may contain secrets or PII under keys the bridge does not treat as sensitive. Combine host tool policy with [MCP host and client logs](#mcp-host-and-client-logs) discipline and the optional allowlist / redaction env vars above. When **`persistence_list_run_events` volume limits** are implemented ([MCP_TOOLS.md § Run event volume limits](MCP_TOOLS.md#run-event-volume-limits-backlog-spec)), they bound **returned** list size and encoded **`events`** size; they do **not** remove the need for allowlists, redaction, or host policy on trusted-operator stores.
...
| `REPLAYT_MCP_BRIDGE_RUN_EVENTS_MAX_COUNT` | **Planned / specified** — when implemented, caps how many events **`persistence_list_run_events`** may return from a single **`load_events`** snapshot (default **10_000**; **`0`** or **negative** disables the count cap). Optional per-call MCP parameters may override for one invocation. Full semantics, tool parameters, and structured error shape: [MCP_TOOLS.md § Run event volume limits](MCP_TOOLS.md#run-event-volume-limits-backlog-spec). Read only in `observability.py` once shipped. |
| `REPLAYT_MCP_BRIDGE_RUN_EVENTS_MAX_TOTAL_BYTES` | **Planned / specified** — when implemented, caps the UTF-8 size of a compact JSON encoding of the loaded **`events`** list (default **32 MiB**; **`0`** or **negative** disables the byte cap). Same doc link as **`REPLAYT_MCP_BRIDGE_RUN_EVENTS_MAX_COUNT`**. Read only in `observability.py` once shipped. |
