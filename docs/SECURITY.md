...
**Bridge code** (`replayt_mcp_bridge`) reads **`REPLAYT_MCP_BRIDGE_*`** variables only in **`observability.py`**: **`REPLAYT_MCP_BRIDGE_LOG_LEVEL`** (verbosity; not a secret); optionally **`REPLAYT_MCP_BRIDGE_STORE_HINT_ROOTS`** (comma-separated absolute paths that constrain explicit `store_hint` values on `persistence_list_run_events`—see below); and optionally **`REPLAYT_MCP_BRIDGE_REDACT_RUN_EVENTS`** (opt-in filtering of returned persistence events—see below). The process still inherits the full OS environment, and **replayt**, **Python**, **HTTP stacks**, and the **MCP parent** may read standard variables listed below.
...
**MCP tool results:** By default, `persistence_list_run_events` returns stored events **as loaded** (pass-through). Operators may set **`REPLAYT_MCP_BRIDGE_REDACT_RUN_EVENTS`** to a truthy value (**`1`**, **`true`**, **`yes`**, **`on`**) so the bridge returns a **redacted copy** of each event (sensitive-shaped dict keys replaced with **`[REDACTED]`** per `redact_structure` in `observability.py`). Either way, payloads may still contain sensitive data under non-matching keys; restrict which MCP clients and users may call that tool, and avoid echoing results into unsecured logging pipelines. The **`replayt_echo`** tool returns the client-supplied string unchanged to the MCP client—do not use it to shuttle secrets, and assume the host may retain that round-trip in traces.
...
- Does **not** redact persistence event bodies **by default**; optional **`REPLAYT_MCP_BRIDGE_REDACT_RUN_EVENTS`** enables key-based redaction on the MCP tool result (see the environment table above).
