...
| `REPLAYT_MCP_BRIDGE_TOOL_TIMEOUT_SECONDS` | **Optional timeout** for selected async tool handlers. When set to a positive number (e.g., `30`), the bridge enforces a wall-clock limit per invocation. If a handler exceeds this duration, it returns a structured error (`status: "error"`, `replayt_surface: "bridge_timeout"`) and logs a `replayt_mcp_bridge.tool.timeout` event. Default: unset (no timeout). Read only in `observability.py`. See [MCP_TOOLS.md](MCP_TOOLS.md) for which tools use this wrapper. |
