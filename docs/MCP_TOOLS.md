...
### Store hint path allowlist (backlog spec)

**Problem:** Explicit **`store_hint`** strings can name any JSONL log directory or SQLite file the server user may open; MCP clients should not double as arbitrary filesystem probes on shared or multi-tenant hosts.

**Mechanism:** Process environment **`REPLAYT_MCP_BRIDGE_STORE_HINT_ROOTS`** (parsed only in **`observability.py`**, like other bridge-owned knobs—see [SECURITY.md § Environment variables](SECURITY.md#environment-variables)). When **unset** or **whitespace-only**, explicit hints are **not** root-restricted. When set to a **non-empty** value with **at least one** parsable absolute root, every **explicit** **`store_hint`** on **`persistence_list_run_events`** must resolve (after the same **`expanduser` / `resolve(strict=False)`** normalization as persistence resolution) to a path **under** one of those roots (**`Path.is_relative_to`**). **Omitted** **`store_hint`** uses replayt’s default log directory and is **not** subject to this allowlist.

**Errors:** Outside-root and unusable-root-configuration failures return the structured object documented in [Error response shape](#error-response-shape) (**`status: "error"`**, **`tool`**, **`replayt_surface`**, **`message`**, **`correlation_id`**). Denial **`message`** strings are **generic**—they **must not** echo the client hint or resolved path. Stderr emits **`replayt_mcp_bridge.store_hint.rejected`** with **`reason`** **`outside_allowlist`** or **`allowlist_unusable`** and no path payload—see [SECURITY.md § Examples: `REPLAYT_MCP_BRIDGE_STORE_HINT_ROOTS`](SECURITY.md#examples-replayt_mcp_bridge_store_hint_roots).

**Examples and operator tables:** [SECURITY.md](SECURITY.md) (capability tier row, variables table, copy-paste **`REPLAYT_MCP_BRIDGE_STORE_HINT_ROOTS`** examples).

**Relationship to typed hints:** **`file:`**, **`jsonl-dir:`**, **`jsonl:`**, and **`sqlite:`** prefixes (see [store_hint grammar](#store_hint-grammar)) resolve to a single filesystem path before allowlist checks—the policy applies uniformly.

**Pytest / closure bar:** [MISSION.md § Optional store path allowlisting for persistence reads (backlog spec)](MISSION.md#optional-store-path-allowlisting-for-persistence-reads-backlog-spec) — at minimum **allowed** and **denied** explicit hints using **temporary directories**.
