...
- **Structured errors:** Operational failures that should reach the client as data use `_tool_error(...)` → `{ status: "error", tool, replayt_surface, message }` (see [MCP_TOOLS.md § Mapped failure paths](MCP_TOOLS.md#mapped-failure-paths-exception--branch-inventory)). `typer.BadParameter` from `load_target` and invalid run IDs are mapped this way instead of leaking stack traces across the MCP boundary. A planned **`correlation_id`** on those objects and on stderr JSON for the same invocation is specified in [MCP_TOOLS.md § Error response shape](MCP_TOOLS.md#error-response-shape); it is **not** in the handler return value yet.
...
- **Tools:** `_log_replayt_tool_boundaries` wraps every registered handler (including `replayt_echo`) and emits JSON lines for `replayt_mcp_bridge.tool.begin` / `.end` with **`tool`**, optional **`mcp_request_id`** from FastMCP `Context`, and result **`status`** on completion. Client argument values are never included. Extra structured fields are redacted via `redact_structure` before emission. **Planned:** add **`correlation_id`** (per [MCP_TOOLS.md § Error response shape](MCP_TOOLS.md#error-response-shape)) on these lines and on `_tool_error` payloads so logs and tool results share one id per invocation.
...
**Follow-ups (product / optional hardening):** **`correlation_id`** on mapped tool errors and stderr logs is **specified** in [MCP_TOOLS.md § Error response shape](MCP_TOOLS.md#error-response-shape) (see [Architecture review: structured tool errors and correlation IDs](#architecture-review-structured-tool-errors-and-correlation-ids)); a separate narrow catch for *additional* unexpected exception classes (beyond today’s mapped set) would still require an explicit inventory update and tests. Optional event field redaction; stricter documentation staging if integrators want a “milestone 1 only” tool exposure story.
...
**Single source of truth:** [MCP_TOOLS.md § Error response shape](MCP_TOOLS.md#error-response-shape) (payload fields, **`correlation_id`** policy, mapped inventory table, unmapped exception rule). [SECURITY.md § MCP host and client logs](SECURITY.md#mcp-host-and-client-logs) explains operator-facing log matching once the field ships.

**Code alignment today:** `server.py` uses `_tool_error` only on the listed mapped paths; `_log_replayt_tool_boundaries` adds **`mcp_request_id`** when available but does not yet emit **`correlation_id`**.
...
**Conclusion:** Specification and acceptance criteria are documented; implementation and correlation-bearing tests are deferred to the next development pass on this backlog item.
