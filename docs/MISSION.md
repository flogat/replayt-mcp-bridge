...
1. **Schemas** — Every string (and bounded list) parameter in [MCP_TOOLS.md — Input shapes](MCP_TOOLS.md#input-shapes-json-schema-concepts) that appears in the [normative bounds table](MCP_TOOLS.md#string-parameter-bounds-backlog-spec) carries the documented **`maxLength`** / **`maxItems`** (and per-element bounds for lists) in the **published** MCP tool JSON Schema (via FastMCP / Pydantic or equivalent). **`replayt_version_info`** remains exempt (empty input object).
...
4. **Pytest** — At minimum: (a) **one over-limit** case that proves **`status: "error"`**, the chosen **`replayt_surface`**, **`correlation_id`**, and **no traceback** in the result dict for a **Tier A** string (see bounds doc); (b) **one over-limit** case for a **large JSON blob** parameter (`inputs_json`, `metadata_json`, `experiment_json`, or `policy_hook_context_json`); (c) **one at-limit** (exactly **N** characters where **N** is the documented maximum) **success** path for a **Tier A** parameter so off-by-one regressions fail CI. Prefer **handler-level** or **FastMCP `call_tool`**-style tests consistent with [`tests/test_mcp_tools.py`](../tests/test_mcp_tools.py); **stdio smoke is optional** for this backlog.
