...
- **Expected failures** — Map to `{ "status": "error", "tool", "replayt_surface", "message" }` as documented in [MCP_TOOLS.md](MCP_TOOLS.md), with a planned **`correlation_id`** field and stderr log alignment per [MCP_TOOLS.md § Error response shape](MCP_TOOLS.md#error-response-shape). Do **not** return raw Python tracebacks inside these objects for the covered failure modes (e.g. `typer.BadParameter` from `load_target`, invalid `run_id`, bad store hints).
