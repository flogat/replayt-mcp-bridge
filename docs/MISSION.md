...
## JSON-schema bounds on high-risk string tool parameters (backlog spec)

**Backlog title:** **Add JSON-schema bounds on high-risk string tool parameters**

**User story:** As an **MCP host author**, I want declared **`maxLength`** (and related JSON Schema bounds) on parameters such as **`target`**, **`store_hint`**, and other high-volume strings so **absurd payloads fail fast** before replayt, JSON parsing, or `pathlib` work consumes unbounded memory or CPU.

**Context:** [MISSION.md § Security and trust boundaries](#security-and-trust-boundaries) already positions **`target`** and **`store_hint`** as **powerful** (imports, filesystem reads). Length and collection caps are a **cheap guardrail** against accidental mega-pastes and naive DoS at the MCP boundary; they **complement** host-side validation, [execution timeouts](MCP_TOOLS.md#execution-timeouts), and [run-event volume limits](MCP_TOOLS.md#run-event-volume-limits-backlog-spec).

**Non-goals:** **Not** a substitute for **OS-level quotas**, **`REPLAYT_MCP_BRIDGE_STORE_HINT_ROOTS`**, host tool policy, or replayt-side semantics. **No** requirement in v1 for **operator-tunable env vars** that raise these caps (a follow-up backlog may add them explicitly). **No** change to **`replayt_version_info`** (no string parameters).

**Intent:** **Mixed** — implement bounds in the **FastMCP / Pydantic** tool surface so **`tools/list` JSON Schemas** advertise **`maxLength`** / **`maxItems`** where applicable, keep **[MCP_TOOLS.md](MCP_TOOLS.md)** as the **integrator copy-of-record** for numeric values, and cover behavior with **pytest** per below.

**Normative parameter table (Builder copy-of-record):** Single table and error semantics live under **[MCP_TOOLS.md § String parameter bounds (backlog spec)](MCP_TOOLS.md#string-parameter-bounds-backlog-spec)**. That section **MUST** stay aligned with this backlog when limits or tool surfaces change.

**Error mapping (normative):** When a client exceeds a documented bound, the bridge **MUST** return the standard structured tool error object from [MCP_TOOLS.md § Error response shape](MCP_TOOLS.md#error-response-shape): **`status: "error"`**, **`tool`**, **`replayt_surface`**, **`message`**, **`correlation_id`** — **no Python traceback** in the returned dict. Use a **stable** handler `replayt_surface` label (**`bridge_input_bounds`**, unless a more specific existing bridge label already applies and is documented in the mapped inventory). **`message` MUST** be **stable English** suitable for operators; it **MUST NOT** echo the **full** offending argument value (avoid log injection and accidental secret exfiltration via error text). Naming the **parameter** (e.g. `target`) and the **kind** of violation (length / array size) is **encouraged**.

**Lifecycle / stderr:** Mapped failures **MUST** still participate in the normal **`replayt_mcp_bridge.tool.begin`** / **`tool.end`** lifecycle with the same **`correlation_id`** as other mapped **`status: "error"`** returns, per [MCP_TOOLS.md § Error response shape — Lifecycle](MCP_TOOLS.md#error-response-shape). If the MCP SDK surfaces argument validation **before** the bridge’s handler runs, the Builder **MUST** either **adjust registration** so validation happens inside a bridge-controlled path that can return the dict shape above, or **document the deviation** under [ARCHITECTURE.md](ARCHITECTURE.md) in the same change-set—**default expectation** is hosts see **schema bounds** in **`tools/list`** **and** over-limit calls get the **structured dict**, not an unmapped transport error.

**String length semantics:** Unless otherwise noted in [MCP_TOOLS.md § String parameter bounds (backlog spec)](MCP_TOOLS.md#string-parameter-bounds-backlog-spec), limits use **Python `len(str)`** on the decoded Unicode string (**code points**), consistent with **Pydantic `max_length`**, and match the JSON Schema emitted for tools.

**Collection semantics / non-requirements:** **`maxItems`** counts decoded list elements (same as Python **`len(list)`**); per-element limits apply independently to each present string item. This backlog adds **upper bounds only**. It does **not** require new **`minLength`**, **`pattern`**, uniqueness, ordering, or semantic JSON validation rules; existing per-tool / replayt behavior still decides what an empty string, duplicate key name, or malformed JSON means **after** the documented size checks pass.

**Original backlog acceptance criteria (traceability):**

- FastMCP / tool schemas document **concrete** bounds; defaults chosen with **headroom** for real workflows.
- Validation errors use the **existing structured error shape**.
- Pytest includes **over-limit** and **at-limit** cases.

**Acceptance criteria (refined, for implementation and review — Builder / Tester):**

1. **Schemas** — Every string (and bounded list) parameter in [MCP_TOOLS.md — Input shapes](MCP_TOOLS.md#input-shapes-json-schema-concepts) that appears in the [normative bounds table](MCP_TOOLS.md#string-parameter-bounds-backlog-spec) carries the documented **`maxLength`** / **`maxItems`** in the **published** MCP tool JSON Schema (via FastMCP / Pydantic or equivalent). For arrays, the published schema **MUST** expose the container **`maxItems`** and the per-element string **`items.maxLength`** (or the equivalent shape under `anyOf` / nullable wrappers). **`replayt_version_info`** remains exempt (empty input object).
2. **Docs** — [MCP_TOOLS.md § String parameter bounds (backlog spec)](MCP_TOOLS.md#string-parameter-bounds-backlog-spec) lists **all** capped parameters with **numeric values** and rationale; per-tool **Input shapes** subsections **MUST** cross-link that section (footnote or “Bounds” row) so tables do not silently drift.
3. **Mapped inventory** — [MCP_TOOLS.md § Mapped failure paths](MCP_TOOLS.md#mapped-failure-paths-exception--branch-inventory) includes at least one row for **`bridge_input_bounds`** (or the chosen label) covering **all** tools whose arguments participate in these caps.
4. **Pytest** — At minimum: (a) **one over-limit** case that proves **`status: "error"`**, the chosen **`replayt_surface`**, **`correlation_id`**, and **no traceback** in the result dict for a **Tier A** string (see bounds doc); (b) **one over-limit** case for a **large JSON blob** parameter (`inputs_json`, `metadata_json`, `experiment_json`, or `policy_hook_context_json`); (c) **one at-limit** (exactly **N** characters where **N** is the documented maximum) **success** path for a **Tier A** parameter so off-by-one regressions fail CI; (d) **one published-schema assertion** against the emitted tool schema (`tools/list` / `mcp.list_tools()` or equivalent) that covers a bounded **array** parameter, proving both **`maxItems`** and per-element **`maxLength`** are visible to hosts. Prefer **handler-level** or **FastMCP `call_tool`**-style tests plus at least one **published-schema** assertion consistent with [`tests/test_mcp_tools.py`](../tests/test_mcp_tools.py); **stdio smoke is optional** for this backlog.
5. **Cross-links** — [SECURITY.md](SECURITY.md) references the bounds section from the trust-boundary / input-hygiene narrative (one sentence or bullet is enough). [CHANGELOG.md](../CHANGELOG.md) **Unreleased** notes the user-visible contract when the Builder ships.

**Changelog:** Add an **Unreleased** bullet when behavior ships (Builder commit).

**Implementation status:** **Shipped** — [MCP_TOOLS.md § String parameter bounds (backlog spec)](MCP_TOOLS.md#string-parameter-bounds-backlog-spec), Pydantic field constraints in [`tools_bounds.py`](../src/replayt_mcp_bridge/tools_bounds.py) and `tools_*.py`, **`BridgeFastMCP.call_tool`** mapping of FastMCP/Pydantic length violations to **`bridge_input_bounds`**, [`tests/test_mcp_tools.py`](../tests/test_mcp_tools.py); workflow phase **3** Builder.
