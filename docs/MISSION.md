...
**Implementation status:** **Partial** — Enforcement and **SECURITY.md** examples / variable table are present in the tree (**`parse_store_hint_allowlist_roots`** in [`observability.py`](../src/replayt_mcp_bridge/observability.py), checks in [`tools_persistence.py`](../src/replayt_mcp_bridge/tools_persistence.py)). **Gap vs refined criteria:** automated tests currently cover an **allowed** explicit hint under a root; an explicit **denied**-path test (and optional rows above) remain for phase **3** / **4** unless already merged elsewhere—see [ARCHITECTURE.md § Architecture review: store_hint root allowlist](ARCHITECTURE.md#architecture-review-store-hint-root-allowlist) **Contract tests** for the accurate inventory.
