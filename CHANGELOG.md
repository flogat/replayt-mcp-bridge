...
- **Stdio MCP session smoke test (spec)** — [docs/MISSION.md](docs/MISSION.md) and [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) now define refined acceptance criteria and architecture review for backlog **“Add an integration smoke test over the real stdio MCP session”** (MCP SDK client, handshake-driven sync, bounded timeouts, preferred tool `replayt_version_info` or `replayt_echo`; implementation tracked as a planned `tests/test_mcp_stdio_session_smoke.py`).
