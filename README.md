...
**Tool exposure:** Use the **[MCP tool capability tiers](docs/SECURITY.md#mcp-tool-capability-tiers)** table in `docs/SECURITY.md` for a single, operator-oriented view of each registered tool (diagnostic vs workflow introspection vs persistence read), filesystem and bridge-owned network implications, and suggested defaults for **local dev** vs **shared workstations**. It cross-links [Security and trust boundaries](docs/MISSION.md#security-and-trust-boundaries) in the mission doc for consistency.
