...
**[docs/SECURITY.md](docs/SECURITY.md)** lists environment variables that affect the bridge and replayt, **what must never be logged** (tokens, PII, raw tool arguments), recommended **local stdio vs remote** deployment patterns, and how **replayt credentials** interact with this process. Read it before exposing the server beyond a trusted local MCP parent. For **stripping inherited environment** on high-assurance hosts (hooks, provider keys, spawn recipes), see **[Minimal environment inheritance](docs/SECURITY.md#minimal-environment-inheritance)**.

**Tool exposure:** Use the **[MCP tool capability tiers](docs/SECURITY.md#mcp-tool-capability-tiers)** table in `docs/SECURITY.md` for a single, operator-oriented view of each registered tool (diagnostic vs workflow introspection vs persistence read), filesystem and bridge-owned network implications, and suggested defaults for **local dev** vs **shared workstations**. For **partial exposure**—how MCP hosts can limit callable tools given the bridge’s fixed registration surface, and what risks remain (including **path- or hint-bearing error messages**)—see **[Host-side partial tool exposure](docs/SECURITY.md#host-side-partial-tool-exposure)**. For **what structured error `message` strings may contain** when sharing transcripts, see **[Structured error messages: paths and operational detail](docs/SECURITY.md#structured-error-messages-paths-and-operational-detail)**. The tier table cross-links [Security and trust boundaries](docs/MISSION.md#security-and-trust-boundaries) in the mission doc for consistency.
...
**Diagnostic echo gate (optional):** To omit **`replayt_echo`** from **`tools/list`**, set **`REPLAYT_MCP_BRIDGE_DISABLE_DIAGNOSTIC_ECHO_TOOLS`** to a truthy token or add **`--no-diagnostic-echo-tools`** before the server starts (not valid with **`health`**). See [docs/SECURITY.md](docs/SECURITY.md#optional-bridge-side-gate-diagnostic-echo-tools).
