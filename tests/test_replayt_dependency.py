...
def test_replayt_installable_and_version_in_range() -> None:
    ...
    assert parts >= (0, 4, 25), (
        f"replayt {ver} below supported floor (see pyproject.toml / DESIGN_PRINCIPLES)"
    )
    assert parts < (0, 5, 0), (
        f"replayt {ver} outside supported upper bound (see pyproject.toml / DESIGN_PRINCIPLES)"
    )
